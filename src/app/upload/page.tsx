...
export default function UploadPage() {
    ...
    const handleDeploy = async () => {
        ...
        try {
            ...
            if (result.success) {
                setMessage({ type: 'success', text: `Pack deployed successfully! URL: ${result.url}` });
            } else {
                setMessage({ type: 'error', text: result.error || 'Deployment failed.' });
            }
        } catch (error) {
            console.error(error);
            setMessage({ type: 'error', text: 'Critical deployment error.' });
        } finally {
            setDeploying(false);
        }
    };
    ...
    const handleManualParse = () => {
        try {
            ...
            setMessage({ type: 'success', text: 'JSON parsed successfully. Ready to deploy.' });
        } catch (error) {
            console.error(error);
    ...
    return (
        <main className="p-8 max-w-6xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-serif font-bold tracking-tight">Pack Deployment Center</h1>
            </div>
                ...
                <Card className="lg:col-span-1">
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="e.g. Geography" />
                        </div>
                        <div className="space-y-2">
                ...
                <Card className="lg:col-span-2">
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={handleManualParse}>Parse & Preview</Button>
                        </div>
                    </CardHeader>
                    <CardContent>
                        <textarea
                    ...
                    <CardFooter className="flex justify-between border-t border-muted/20 pt-6">
                        ...
                        >
                            {deploying ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}
            ...
            {currentPack && (
                <div className="bg-card border rounded-xl p-6">
                    <h3 className="text-xl font-bold mb-4">Content Preview: {currentPack.title}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {currentPack.questions?.map((q, i) => (
                            <div key={q.id || i} className="p-4 border rounded-lg bg-background/50">
                                <p className="font-semibold mb-2">{i+1}. {q.text}</p>
                                <div className="grid grid-cols-2 gap-2">
                                    {q.options.map((opt, idx) => (
                                        <div key={idx} className={`text-xs p-2 rounded ${idx === q.correctAnswerIndex ? 'bg-green-500/10 text-green-600 border border-green-500/20' : 'bg-muted/50'}`}>
                                            {opt}
                                        </div>
                                    ))}
                                </div>
